...
| `onAutoSaveNeeded` | | User changed the chart. `Chart change` means any user action that can be undone. The callback function will not be called more than once every 5 seconds. See also [auto_save_delay](wiki/charting_library/Widget-Constructor.md#auto_save_delay) |
| `onScreenshotReady` | | A screenshot URL is returned by the server |
| `onMarkClick` | | User clicked a [mark on a bar](wiki/charting_library/Marks-On-Bars.md). Mark ID will be passed as an argument |
...
Returns a chart object that you can use to call [Chart-Methods](wiki/charting_library/Chart-Methods.md)

### setLanguage(locale)

1. `locale`: [language code](wiki/charting_library/Localization.md)
...
1. `drawingId`: may be one of the [identifiers](wiki/charting_library/Shapes-and-Overrides.md) or
    1. `cursor`
    1. `dot`
    1. `arrow_cursor`
    1. `eraser`
...
Returns an [identifier](wiki/charting_library/Shapes-and-Overrides.md) of the selected drawing or cursor (see above).
...
This method returns a [WatchedValue](wiki/charting_library/WatchedValue.md) object that can be used to read/set/watch the state of Lock All Drawing Tools button.
...
This call is part of the low-level [save/load API](wiki/charting_library/Saving-and-Loading-Charts.md).

### load(state)

1. `state`: object

Loads the chart from the `state` object. This call is part of the low-level [save/load API](wiki/charting_library/Saving-and-Loading-Charts.md).
...
Starting from version `1.4` use [custom_css_url](wiki/charting_library/Widget-Constructor.md#custom_css_url) instead.
...
1. `overrides` is an object.
    It is the same as [overrides](wiki/charting_library/Widget-Constructor.md#overrides) in the Widget Constructor.
...
1. `overrides` is an object. It is the same as [studies_overrides](wiki/charting_library/Widget-Constructor.md#studies_overrides) in the Widget Constructor.
...
The following methods are available in [Trading Terminal](wiki/charting_library/Trading-Terminal.md) only.
...
Returns a chart object of the active chart that you can use to call [Chart-Methods](wiki/charting_library/Chart-Methods.md)
...
* [Chart-Methods](wiki/charting_library/Chart-Methods.md)
