:chart: All content on this page applies to [Trading Terminal](wiki/charting_library/Trading-Terminal.md) only.
...
This is an object that should be passed in the constructor of the Trading Terminal to [brokerConfig](wiki/charting_library/Widget-Constructor.md#brokerConfig). Each field should have a boolean value (`true`/`false`):
...
* `supportPLUpdate`
    ...
    Broker provides PL for a position. If the broker calculates profit/loss by itself it should call [plUpdate](wiki/charting_library/Trading-Host.md#plupdatepositionid-pl) as soon as PL is changed.
