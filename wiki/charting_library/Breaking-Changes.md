...
- Action `takeScreenshot` from [executeActionById](wiki/charting_library/Chart-Methods.md#executeactionbyidactionid) method is removed. Use [takeScreenshot](wiki/charting_library/Widget-Methods.md#takescreenshot) method instead.
- Action `lockDrawingsAction` from [executeActionById](wiki/charting_library/Chart-Methods.md#executeactionbyidactionid) and [getCheckableActionState](wiki/charting_library/Chart-Methods.md#getcheckableactionstateactionid) method are removed. Use [lockAllDrawingTools](wiki/charting_library/Widget-Methods.md#lockalldrawingtools) instead.
- Featureset `caption_buttons_text_if_possible` is enabled by default.
- Fixed an [issue](wiki/charting_library/https://github.com/tradingview/charting_library/issues/2652.md) that was causing bars to shift. Time-shifted bars used to appear when daily bars had a negative exchange timezone offset along with a 24x7 session. If you have a workaround for this issue, please remove it before updating to this version.

## Version 1.12

**Charting Library**

- `charting_library/charting_library.min.js` is now [UMD](wiki/charting_library/https://github.com/umdjs/umd.md) module.
...
- Overrides for `Overlay` should be applied only via `studies_overrides` (and `applyStudiesOverrides` in runtime). In the previous versions you had to use `overrides` and `applyOverrides`). See [Studies-Overrides](wiki/charting_library/Studies-Overrides.md) page.
...
**The Trading Controller is replaced with [Broker API](wiki/charting_library/Broker-API.md)**.
...
- Method `buttonDropdownItems` is removed. Broker API should update [Broker API](wiki/charting_library/Trading-Host.md) using `setButtonDropdownActions`.
- Methods `configFlags` and `durations` are removed. Use [Widget Constructor](wiki/charting_library/Widget-Constructor.md) fields instead.
...
- `tradingController` field in the [Widget Constructor](wiki/charting_library/Widget-Constructor.md) is removed. Use `brokerFactory` instead.
...
- `showSampleOrderDialog` removed, use [showOrderDialog](wiki/charting_library/Trading-Host.md#showorderdialogorder-handler-focus-promise) instead
- `showOrderDialog` removed from [Broker API](wiki/charting_library/Broker-API.md), use `placeOrder` and `modifyOrder` receive `silently` argument instead
