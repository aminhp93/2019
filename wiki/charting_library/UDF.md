...
Response: Library expects to receive a JSON response of the same structure as a result of JS API [setup() call](wiki/charting_library/JS-Api.md#onreadycallback).
...
Each property is treated as table column, as described above (see [response-as-a-table](wiki/charting_library/UDF.md#response-as-a-table-concept)).
The response structure is similar (but **not equal**) to [SymbolInfo](wiki/charting_library/Symbology.md#symbolinfo-structure) so please read the description to learn about the details of each field.
...
A JSON response of the same structure as [SymbolInfo](wiki/charting_library/Symbology.md#symbolinfo-structure)
...
* `type`: string. One of the symbol types [supported](wiki/charting_library/JS-Api.md#symbols_types) by your back-end
* `exchange`: string. One of the exchanges [supported](wiki/charting_library/JS-Api.md#exchanges) by your back-end
* `limit`: integer. The maximum number of symbols in a response

Example: `GET /search?query=AA&type=stock&exchange=NYSE&limit=15`

A response is expected to be an array of symbol objects as in [respective JS API call](wiki/charting_library/JS-Api.md#searchsymbolsuserinput-exchange-symboltype-onresultreadycallback)
...
Charting Library aligns the time according to the [Session](wiki/charting_library/Symbology.md#session) from SymbolInfo.
...
This object is similar to [respective response](wiki/charting_library/JS-Api.md#getmarkssymbolinfo-startdate-enddate-ondatacallback-resolution) in JS API, but each property is treated as a table column, as described above.
...
* `errmsg`: Error message. Should be present only when `s = 'error'`
* `d`: [symbols data](wiki/charting_library/Quotes.md) Array
