...
**Starting from version 1.5.** You can call these methods using chart object returned to you by widget's methods [chart(index)](wiki/charting_library/Widget-Methods.md#chart-chartindex) or [activeChart()](wiki/charting_library/Widget-Methods.md#chart-activechart).
...
You can subscribe using [Subscription](wiki/charting_library/Subscription.md) object returned by this function to be notified when new history bars are loaded. You can also use the same object to unsubscribe from the event.

### onSymbolChanged()

You can subscribe using [Subscription](wiki/charting_library/Subscription.md) object returned by this function to be notified when the symbol is changed. You can also use the same object to unsubscribe from the event.

### onIntervalChanged()

You can subscribe using [Subscription](wiki/charting_library/Subscription.md) object returned by this function to be notified when the interval is changed. You can also use the same object to unsubscribe from the event.
...
1. `resolution`: string. Format is described in another [article](wiki/charting_library/Resolution.md).
1. `callback`: function()
...
Before calling this function you should call [onResetCacheNeededCallback](wiki/charting_library/JS-Api.md#subscribebarssymbolinfo-resolution-onrealtimecallback-subscriberuid-onresetcacheneededcallback).
...
See [timezone](wiki/charting_library/Widget-Constructor.md#timezone) for more information.
...
1. `overrides`: (starting from version `1.2`) an object [containing properties](wiki/charting_library/Studies-Overrides.md) you'd like to set for your new study. Note that you should not specify the study name. Start a property path with a plot name.
...
Returns an instance of the [StudyApi](wiki/charting_library/Study-Api.md) that allows you to interact with the study.
...
1. `options`: object `{shape, [text], [lock], [overrides]}`
    * `shape` may be one of the [identifiers](wiki/charting_library/Shapes-and-Overrides.md)
    * `text` is an optional argument. It's the text that will be included in the shape if it's supported.
    * `lock` shows whether a user will be able to remove/change/hide the shape or not.
    * `disableSelection` prevents selecting of the shape
    * `disableSave` prevents saving the shape on the chart
...
Check out [Shapes and Overrides](wiki/charting_library/Shapes-and-Overrides.md) for more information.
...
Returns an instance of the [ShapeApi](wiki/charting_library/Shape-Api.md) that allows you to interact with the shape.
...
Returns an array of instances of the [PaneApi](wiki/charting_library/Pane-Api.md) that allows you to interact with the panes.
...
This call is a part of low-level [save/load API](wiki/charting_library/Saving-and-Loading-Charts.md).
...
It is strongly recommended to read [this article](wiki/charting_library/Trading-Primitives.md) before using this call.
...
Returns the chart's time interval. The format is described in this [article](wiki/charting_library/Resolution.md).
...
* [Widget Constructor](wiki/charting_library/Widget-Constructor.md)
* [Saving and Loading Charts](wiki/charting_library/Saving-and-Loading-Charts.md)
* [Overriding Studies' Defaults](wiki/charting_library/Studies-Overrides.md)
* [Overriding Chart's Defaults](wiki/charting_library/Overrides.md)
